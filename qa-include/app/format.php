...
function qa_post_html_fields($post, $userid, $cookieid, $usershtml, $dummy, $options = array())
{
	...
	if ($isquestion) {
		...
		if (@$options['answersview'] && isset($post['acount'])) {
			...
			$fields['answers'] = stw_num_word_skl($post['acount'], array(qa_lang_html('main/stw_x_answers_1'), qa_lang_html('main/stw_x_answers_2'), qa_lang_html('main/stw_x_answers_3')));
	...
	if (@$options['avatarsize'] > 0) {
		if (QA_FINAL_EXTERNAL_USERS)
			...
			$fields['avatar'] = qa_get_user_avatar_html(@$post['flags'], @$post['email'], @$post['userid'],
...
function qa_message_html_fields($message, $options = array())
{
	...
	if (@$options['towhomview']) {
		...
		$fields['avatar'] = qa_get_user_avatar_html(@$message['toflags'], @$message['toemail'], @$message['touserid'],
		...
		if (@$options['avatarsize'] > 0) {
			$fields['avatar'] = qa_get_user_avatar_html(@$message['fromflags'], @$message['fromemail'], @$message['fromuserid'],
...
function qa_other_to_q_html_fields($question, $userid, $cookieid, $usershtml, $dummy, $options)
{
	...
	if (@$options['avatarsize'] > 0) {
		if (QA_FINAL_EXTERNAL_USERS)
			...
			$fields['avatar'] = qa_get_user_avatar_html($question['oflags'], $question['oemail'], $question['ouserid'],
